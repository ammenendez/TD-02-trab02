...
utilizando dados de população, nascimentos e óbitos referentes ao Distrito Federal. O código, os 
dados processados e os resultados estão disponíveis no repositório 
...
Os valores estimados para os dois indicadores são apresentados na Tabela~\ref{tab:tbr_tlr}.
...
\end{equation}
Para o Distrito Federal, obteve-se \(T_0=29{,}33\) anos. Com \(TLR=0{,}7336\), a aproximação resultou em \(r_0=-0{,}0105622\) ao ano. Os valores \(-1{,}0562\%\) e \(-10{,}5622\) por mil apresentados adiante correspondem, respectivamente, a \(100r_0\) e \(1000r_0\).
...
Os resultados obtidos pelos dois procedimentos são apresentados na Tabela~\ref{tab:r_intrinseco}.
...
\begin{tabular}{lrr}
\toprule
Método & $100r$ (\% ao ano) & $1000r$ (por mil ao ano) \\  
\midrule
Aproximação pela TLR & $-1{,}0562$ & $-10{,}5622$ \\
Equação de Euler--Lotka & $-1{,}0474$ & $-10{,}4741$ \\
\bottomrule
\end{tabular}
...
mil ao ano na escala \(1000r\). A solução da equação de Euler--Lotka, \(r=-0{,}0104741\), foi adotada nos cálculos 
subsequentes. O sinal negativo indica que as condições de fecundidade e mortalidade de 2022 
implicariam decréscimo anual efetivo de aproximadamente 1,04\% em uma população estável e fechada. 
...
A taxa bruta de natalidade da população estável expressa o número anual de nascimentos por unidade de população na estrutura etária correspondente ao valor intrínseco de \(r\). Para incluir ambos os sexos, foram consideradas as proporções masculina e feminina observadas entre os nascimentos e as funções de sobrevivência das respectivas tábuas de vida. O cálculo foi realizado por \cite{grupofoz2021,preston2001}:
...
estrutura resultasse em 100\% \cite{grupofoz2021,preston2001}. A distribuição resultante é apresentada na Figura~\ref{fig:estrutura_lotka}.
...
estrutura consideravelmente mais envelhecida que a observada no Censo de 2022. Essa estrutura é apresentada na Figura~\ref{fig:estrutura_leslie}.
...
A estrutura observada apresenta maior concentração nas idades adultas, principalmente entre 20 e 49 anos, e participação relativamente reduzida nas idades mais avançadas. Esse formato é compatível com a redução recente da fecundidade e com o histórico de atração migratória do Distrito Federal sobre indivíduos em idade economicamente ativa; contudo, a decomposição desses efeitos não integra o escopo deste trabalho.

A comparação por grandes grupos etários é sintetizada na Tabela~\ref{tab:comparacao_grupos}.

\begin{table}[H]
\centering
\caption{Distribuição da população por grandes grupos etários, segundo a estrutura considerada, Distrito Federal, 2022}
...
A comparação detalhada por grupos quinquenais é apresentada na Figura~\ref{fig:comparacao_estruturas}.

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]
{../figuras/comparacao_estruturas_etarias_df_2022.png}
\caption{Comparação entre a estrutura etária observada e as estruturas estáveis estimadas, Distrito Federal, 2022}
...
Os indicadores da trajetória simulada são apresentados na Tabela~\ref{tab:momentum}.
...
A trajetória relativa ao tamanho populacional de 2022 é apresentada na Figura~\ref{fig:momentum}.
...
produziu $100r=-1{,}0474\%$ ao ano. A proximidade entre as estimativas indica que a aproximação pela 
idade média da função líquida de maternidade, estimada em 29,33 anos, representou adequadamente o regime reprodutivo 
